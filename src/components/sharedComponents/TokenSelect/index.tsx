...
const Wrapper = styled.div.attrs(({ className = 'tokenSelectWrapper' }) => {
  ...
  box-shadow: var(
    --theme-token-select-background-color,
    0 9.6px 13px 0 rgb(0 0 0 / 8%)
  );
  display: flex;
  flex-direction: column;
  max-width: calc(100vw - var(--base-gap-xl, 16px));
  padding: calc(var(--base-common-padding, 8px) * 5) 0
    calc(var(--base-common-padding, 8px) * 3);
...
const TokenSelect = withSuspenseAndRetry<Props>(
  ({
    ...
    const { appChainId, walletChainId, evmWalletChainId } = useWeb3Status()
    ...
    const [chainId, setChainId] = useState<Chain['id']>(() =>
      getValidChainId({
        ...
        dappChains: chains,
        networks,
        walletChainId: evmWalletChainId,
      }),
    )
    ...
    useEffect(() => {
      ...
      const currentDeps = [appChainId, currentNetworkId, evmWalletChainId]
    ...
    }, [appChainId, currentNetworkId, networks, walletChainId, evmWalletChainId])
